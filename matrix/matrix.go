...
func Matr1x() {
	...
	for i := range verse {
		verse[i] = make([]byte, 8)
		for j := range verse[i] {
			verse[i][j] = byte(rng.Intn(2))
		}
	}
	for i := 0; i < 33; i++ {
		for j := 0; j < 8; j++ {
			for k := 0; k < 8; k++ {
				cells[j].Tape[k] = verse[j][k]
			}
			cells[j].Step(rng)
			for k := 0; k < 8; k++ {
				verse[j][k] = cells[j].Tape[k]
			}
		}
		for j := 0; j < 8; j++ {
			for k := 0; k < 8; k++ {
				cells[8+j].Tape[k] = verse[k][j]
			}
			cells[8+j].Step(rng)
			for k := 0; k < 8; k++ {
				verse[k][j] = cells[8+j].Tape[k]
			}
		}
		for j := 0; j < 8; j++ {
			for k := 0; k < 8; k++ {
				fmt.Printf("%d", verse[j][k])
			}
			fmt.Println()
		}
		fmt.Println()
